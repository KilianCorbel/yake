...
class AlbumWindow extends Component {
  ...
  componentDidMount() {
    let params = new URLSearchParams(
      window.location.href.split("/showAlbum")[1]
    );
  ...
  render() {
    ...
    if (
      ...
      this.state.album.nom !== undefined &&
      this.state.album.musiques !== undefined &&
      this.state.album.musiques.length > 0
    ) {
      musiqueList = this.state.album.musiques.map(ele => {
        ele.nomAlbum = this.state.album.nom;
        ele.idAlbum = this.state.album._id;
        ele.nomGroupe = this.state.album.nomGroupe;
        ele.idGroupe = this.state.album.idGroupe;
        return ele;
      });
      ...
      musiqueList = (
        <div>
          <h3>Musiques contenues dans cet album</h3>
          <div className="MusicList">{musiqueList}</div>
        </div>
      );
      retour = (
          ...
          <div className="HeaderAlbumWindow">
            <div className="AlbumTitle">
              ...
              <Link
                ...
                to={`/showArtiste?id=${this.state.album.idGroupe}`}
              >
                <h5 className="clicable">{`${this.state.album.nomGroupe}`}</h5>
              </Link>
              <p>
                {this.state.album.genres.reduce(
                  (accu, ele, ind) =>
                    this.state.album.genres.length - 1 === ind
                      ? accu + ele
                      : accu + ele + ",",
                  ""
                )}
            ...
            <div className="AlbumImg">
              <img
                src={`api/artistes/albums/stream/${this.state.album._id}`}
                alt="noImage"
                key={"album"}
              />
            </div>
...
<div></div>          
      ...
      retour = <div className="scrollable">Cet album n'existe pas</div>;
    }
    return <div className="MainContent">{retour}</div>;
  }
}
